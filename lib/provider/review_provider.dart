...
class ReviewProvider with ChangeNotifier {
  ...
  double getAverageRating(String productId) {
    ...
    return productReviews.fold<double>(0, (sum, item) => sum + item.rating) / productReviews.length;
  }
  ...
  Future<void> addReview(String productId, String userId, double rating, String reviewText) async {
    try {
      ...
      print('Adding review to database: $newReview');
      final key = await _store.add(db, newReview);
      print('Review added with key: $key');
      _reviews.add(Review(
        id: key.toString(),
        productId: productId,
        userId: userId,
